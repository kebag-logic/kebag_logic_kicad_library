...
def extract_subsheet_files(sch_path: Path):
    """
...
def extract_sheets(root_schematic):
    """
    ...
    while to_process:
        ...
        # Extract subsheets
        subsheets = extract_subsheet_files(current)
...
def extract_symbols(sexp, field_prefix="I_") -> List[Tuple[str, Dict[str, float]]]:
    results_dict = {}
                ...
                if isinstance(item, list) and len(item) >= 3:
                    if isinstance(item[0], sexpdata.Symbol) and str(item[0]) == "property":
                        ...
                        if prop_name == "Reference":
                            ref = prop_value
                        elif prop_name.startswith(field_prefix):
                            try:
...
def extract_symbols_from_file(sch_path: str, field_prefix="I_") -> List[Tuple[str, Dict[str, float]]]:
    path = Path(sch_path).resolve()
    sexp = parse_kicad_sch(path)
    return extract_symbols(sexp, field_prefix)

def generate_markdown_table(all_components: List[Tuple[str, str, Dict[str, float]]]) -> str:
    """
    ...
    filtered_components = [(ref, value, currents) for ref, value, currents in all_components if not ref.startswith("L")]
...
def save_markdown_to_file(md_text: str, file_path="power_budget.md"):
    """
...
def collect_all_components(all_sheets: List[str], field_prefix="I_") -> List[Tuple[str, str, Dict[str, float]]]:
    """
    ...
    for sheet in all_sheets:
        ...
        sheet_components = extract_symbols_from_file(sheet, field_prefix=field_prefix)
